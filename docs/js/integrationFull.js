$(document).ready(function() {
  ...
  const contractAddress = '0xAe7606C84e13455Fc53C4c83E2f36c4A3114394d'

  const totalInvestorsEvent = ''
  const lastDepositEvent = ''
  ...
  function getUserInfo(address) {
    ...
    contract.methods.investors(userAddress).call({from: '0xAe7606C84e13455Fc53C4c83E2f36c4A3114394d'}, function(error, result) {
      ...
      contract.methods.bonusSize().call({from: '0xAe7606C84e13455Fc53C4c83E2f36c4A3114394d'}, function(error, result) {
        const timeout = Math.floor(((deposit * 2) / (deposit * (result / 100))) * 10000) / 10000
        $('.user-forecast').html( timeout + ' days' )
      })
    })

    contract.methods.payoutSize(userAddress).call({from: '0xAe7606C84e13455Fc53C4c83E2f36c4A3114394d'}, function(error, result) {
